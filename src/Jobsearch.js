...
export default function Search() {
    ...
    return (
        <>
             ...
             <hr />
            {data.results?.map(item => <p key = {item.id}> { item.title } : { item.description } | More information can be found at <a href={item.redirect_url} className='Jobs'>{ item.redirect_url }</a></p>)}
        </>
    );
}